...
# Stones that ar elinked by a chain of connected stones of the same color
class GoString():
...
class Board():
    ...
    # Returns None if the point is emtpy, or a Player if there is a stone on that point
    def get(self, point):
    ...
    def __deepcopy__(self, memodict={}):
        ...
        # Can do a shallow copy b/c the dictionary maps tuples (immutable)
        # to GoStrings (also immutable)
...
# Any action a play can play on a turn
class Move():
    ...
    # Move that places a stone on the board
    @classmethod
    def play(cls, point):
...
class GameState():
    ...
    # Return the new GameState after applything the move
